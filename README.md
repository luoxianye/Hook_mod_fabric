...
一个基于 Fabric 的 Minecraft 勾爪 Mod。玩家可以将勾爪装备到 Trinkets 的 `offhand/hook` 槽位中，并通过可自定义按键发射勾爪、切换勾爪模式、释放拉取中或固定中的勾爪。
...
- 新增跳跃键松开模式 B 勾爪。
- 模式 B 拉取中提前松开时会保留当前速度动能。
- 模式 B 固定悬挂后松开时会清空速度，避免突然弹飞。
- 模式 A 增加卡住检测，修复复杂方块夹角中短暂悬浮的问题。
- 固定悬挂提示支持动态显示玩家当前绑定按键。
...
- 装备槽位：`offhand/hook`
- 默认按 `R` 使用勾爪 / 松开模式 B 中的勾爪
- 默认按 `G` 切换勾爪功能模式
- 支持使用跳跃键松开模式 B 中的勾爪
    - 默认跳跃键为空格
    - 如果玩家修改了跳跃键，松开操作会跟随新的跳跃键
- `R`、`G` 与跳跃键均支持在游戏控制设置中自定义
- 支持两种勾爪功能模式：
    - **模式 A：普通拉取**  
      保持原本逻辑。勾爪命中方块后将玩家拉向命中点，随后勾爪释放。  
      当玩家被复杂方块结构卡住时，会提前结束拉取，避免玩家短暂悬浮。
    - **模式 B：固定悬挂**  
      勾爪命中方块后将玩家拉向命中点，勾爪不会立即松开。玩家到达后会被固定在勾爪命中位置附近。  
      玩家可以按 `R` 或跳跃键松开；如果在尚未到达终点时提前松开，会保留当前速度动能。
- 切换功能模式时会显示模式提示
- 固定悬挂与松开勾爪时会显示状态提示
- 固定悬挂提示会动态显示玩家当前绑定的按键
- 右键不再触发勾爪
- 未装备勾爪时按键不会显示提示信息
- 勾爪投射物支持命中方块与实体
- 命中方块后将玩家沿直线拉向目标点
...
6. 在模式 B 中，勾爪命中方块后会将玩家拉向目标点。
7. 在模式 B 拉取过程中，可以按 `R` 或跳跃键提前松开；此时玩家会保留当前速度动能。
8. 在模式 B 到达目标点并固定悬挂后，可以按 `R` 或跳跃键松开；此时会清空速度，避免突然弹飞。
9. 可以在 `选项 -> 控制 -> 按键绑定 -> Hook Mod` 中修改 `R` 与 `G` 对应按键，也可以在控制设置中修改跳跃键。
...
v2.1.1 起，模式 A 加入卡住检测。玩家如果被头顶、前方、侧方等复杂方块结构限制，且连续数 tick 没有明显接近目标点，拉取任务会提前结束，避免持续施加拉力导致玩家悬浮。
...
v2.1.1 起，模式 B 支持两种释放时机：

- **拉取中提前释放**：玩家尚未到达命中点时，按 `R` 或跳跃键可以松开勾爪，并保留当前速度动能。
- **固定后释放**：玩家已经固定悬挂后，按 `R` 或跳跃键可以松开勾爪，并清空速度，避免突然弹飞。
...
| `R`              | 使用勾爪 / 松开模式 B 中的勾爪        |
| `G`              | 切换勾爪模式                          |
| 跳跃键，默认空格 | 松开模式 B 中的勾爪，不会触发发射勾爪 |

提示文本中的按键名称会根据玩家当前绑定自动变化。例如玩家把 `R` 改成 `U`，固定悬挂提示中也会显示 `U`。
...
```json
{
  ...
  "message.hook.anchored": "勾爪已固定，请使用按键 %s 或 %s 松开"
}
```
...
{
  "key.category.hook.controls": "Hook Mod",
  "key.hook.use": "Use Hook",
  "key.hook.toggle_mode": "Switch Hook Mode",
  "message.hook.anchored": "Hook anchored. Press %s or %s to release"
}
```

固定悬挂提示建议通过 `Component.keybind(...)` 动态显示当前按键，而不是写死 `R` 或空格。
...
1. 如果玩家正在模式 B 拉取中，则松开勾爪，并保留当前速度动能。
2. 如果玩家正在模式 B 固定悬挂中，则松开勾爪，并清空速度。
3. 如果玩家没有处于拉取或悬挂状态，则检查玩家是否装备了勾爪。
4. 检查勾爪是否处于冷却。
5. 添加冷却。
6. 发射 `HookProjectileEntity`。
...
```text
pulling -> no progress / collision stuck -> release
```

当玩家连续数 tick 没有明显接近目标点，或者在碰撞状态下无法继续前进时，应提前结束拉取，避免玩家在复杂方块夹角中悬浮。

模式 B 中，`PlayerPullManager` 需要维护拉取、固定与释放状态：

```text
pulling -> arrive -> anchored -> release
```

v2.1.1 起，模式 B 支持拉取中提前释放：

```text
pulling -> release with current velocity
anchored -> release with zero velocity
```

推荐将释放逻辑拆分为：

```java
PlayerPullManager.releaseForUseKey(player, true);
PlayerPullManager.releaseModeBOnly(player, true);
```

其中：

- `releaseForUseKey` 用于 `R` / 使用勾爪键。
- `releaseModeBOnly` 用于跳跃键，只释放模式 B，不触发发射勾爪。
- 模式 B 拉取中释放时保留速度。
- 模式 B 固定后释放时清空速度。
- 模式 A 拉取中释放时保持原有安全逻辑。
...
如果本次更新包含功能切换与固定悬挂，推荐版本号为：

```text
v2.1.0
```

如果是在 v2.1.0 基础上新增跳跃键松开、模式 B 提前松开保留动能，并修复模式 A 卡住检测，推荐版本号为：
